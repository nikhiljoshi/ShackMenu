...
class SubScrollView extends HorizontalScrollView {
    ...
    private void did(boolean show){
        for (int i = 0; i < mContainer.getChildCount(); i++){
            ...
            if (child instanceof ShackItemView){
                if (show) {
                    ((ShackItemView) child).didShow();
                }else{
                    ((ShackItemView) child).didDismiss();
                }
            }
        }
    }
    ...
    public void showItemAnimation(){
        ...
        for (int i = 0; i < mContainer.getChildCount(); i++){
            ...
            if (child.getGlobalVisibleRect(new Rect()) && child instanceof ShackItemView){
                ShackItemView itemView = (ShackItemView) child;
                ...
                if (animation != null) {
                    ((ShackItemView) child).showAnimation(animation);
                }else{
                    ((ShackItemView) child).showAnimator(animator);
                }
